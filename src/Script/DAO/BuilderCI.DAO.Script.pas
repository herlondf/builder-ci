...
uses
  ...
  Winapi.Windows,
  CI.Help,
  BuilderCI.DAO.Script.Interfaces;

type
  TScript = class(TInterfacedObject, iDAOScript)
    constructor Create;
...
begin
  ...
  if ( ExtractFileExt( FFileParam ) <> '.txt' ) then
  begin
    if ( ExtractFileExt( FFileParam ) <> '.sql' ) then
    begin
      Writeln(' BuilderCI - Extensao ' + ExtractFileExt( FFileParam ) + ' desconhecida.');
      Writeln(' BuilderCI - Erro! So pode ser enviado arquivos com extensao (*.txt, *.sql).');
      Readln( FResult );
      Exit;
    end;
  end;

  AssignFile ( FArqScriptFinal, ExtractFilePath( FFileParam ) + FTicket + '-Temp.txt' );
  Rewrite( FArqScriptFinal );

  WriteLN(FArqScriptFinal, UTF8Encode( ' -- ============================' + FTicket + ' ============================' )                                );
  WriteLN(FArqScriptFinal, UTF8Encode( ' -- Desenvolvedor: ' + FDeveloper                                             )                                );
  WriteLN(FArqScriptFinal, UTF8Encode( ' -- Sprint.......: ' + FSprint                                                )                                );
  WriteLN(FArqScriptFinal, UTF8Encode( ' -- Demanda......: ' + FTicket                                                )                                );
  WriteLN(FArqScriptFinal, UTF8Encode( ' -- URL..........: https://viggosistemas.acelerato.com/tickets/' + FTicket    )                                );
  WriteLN(FArqScriptFinal, UTF8Encode( ' -- -----------------------------' + StringOfChar('=', Length( FTicket ) ) + '============================' )  );

  for I := 0 to Pred( Lista.Count ) do
    Writeln( FArqScriptFinal, UTF8Encode(' ') + UTF8Encode( Lista.Strings[I] ) );

  WriteLN(FArqScriptFinal, UTF8Encode( ' -- ============================ ' + FTicket + ' ============================' )                               );


  CloseFile( FArqScriptFinal );

  Lista.Free;

  WriteLN(' BuilderCI - Enviando o arquivo...');

  WriteLN( FFTP.MakeDir(FSprint) );

  Envio:

  if FFTP.FileExists( FTicket + '.txt' ) then
  begin
    WriteLN(' BuilderCI - Ops... O arquivo ja existe no servidor. ');
    Write  (' BuilderCI - Deseja sobrescrever(S/N)? '              );
    ReadLN (FResult);
  end;

  if AnsiUpperCase( FResult ) = 'N' then
  begin
    WriteLN(' BuilderCI - Certo, ate mais... ');
    ReadLN (FResult);
    Exit;
  end;

  if FFTP.SendFile( ExtractFilePath( FFileParam ) + FTicket + '-Temp.txt', FTicket + '.txt' ) then
  begin
...
begin
  ...
  begin
    ...
    for I := 0 to Pred( FLista.Count ) do
      Writeln( arqLatestFinal, UTF8Encode(' ') + UTF8Encode( FLista.Strings[I] ) );
